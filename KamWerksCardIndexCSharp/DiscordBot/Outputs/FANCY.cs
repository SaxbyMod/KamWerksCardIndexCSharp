...
namespace KamWerksCardIndexCSharp.DiscordBot.Outputs
{
	public class FANCY
	{
		public async static Task<(DiscordMessageBuilder mess, string takeout)> CTI(DiscordMessageBuilder messageBuilder, int iterator30, string input, string[] formattedcontent)
		{
			...
			try
			{
				Portrait = await httpClient.GetStreamAsync(PortraitURLBase + formattedcontent[1].TrimStart().Replace(" ", "").Replace("\n", "") + ".png");
			}
					...
					portraitImage.Mutate(img =>
						{
							...
							else if (portraitImage.Width == 86 && portraitImage.Height == 59)
							{
								...
								float tolerance = 0.1f; // Tolerance for color matching

								portraitBase.Mutate(ctx =>
								{
									ctx.ProcessPixelRowsAsVector4((accessor, rows) =>
									{
										for (int y = 0; y < portraitBase.Height; y++)
										{
											Span<Vector4> pixelRow = accessor;
											for (int x = 0; x < pixelRow.Length; x++)
											{
												if (IsWithinTolerance(pixelRow[x], baseLight.ToVector4(), tolerance))
												{
													if (ctiProperties[4] == "Common" || ctiProperties[4] == "Side-Deck" || ctiProperties[4] == "Common (Joke Card)")
													{
														pixelRow[x] = commonLight.ToVector4();
													}
													if (ctiProperties[4] == "Rare" || ctiProperties[4] == "Talking")
													{
														pixelRow[x] = rareLight.ToVector4();
													}
												}
												if (IsWithinTolerance(pixelRow[x], baseMid.ToVector4(), tolerance))
												{
													if (ctiProperties[4] == "Common" || ctiProperties[4] == "Side-Deck" || ctiProperties[4] == "Common (Joke Card)")
													{
														pixelRow[x] = commonMid.ToVector4();
													}
													if (ctiProperties[4] == "Rare" || ctiProperties[4] == "Talking")
													{
														pixelRow[x] = rareMid.ToVector4();
													}
												}
												if (IsWithinTolerance(pixelRow[x], baseDark.ToVector4(), tolerance))
												{
													if (ctiProperties[4] == "Common" || ctiProperties[4] == "Side-Deck" || ctiProperties[4] == "Common (Joke Card)")
													{
														pixelRow[x] = commonDark.ToVector4();
													}
													if (ctiProperties[4] == "Rare" || ctiProperties[4] == "Talking")
													{
														pixelRow[x] = rareDark.ToVector4();
													}
												}
											}
										}
								...
								bool IsWithinTolerance(Vector4 color, Vector4 target, float tolerance)
								{
									return Math.Abs(color.X - target.X) <= tolerance &&
										Math.Abs(color.Y - target.Y) <= tolerance &&
										Math.Abs(color.Z - target.Z) <= tolerance;
								}
